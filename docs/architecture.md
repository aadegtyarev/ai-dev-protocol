...
The boundary deny does **not yet** consult this set — the validator lands proven before it gains power to widen a deny (the staged rollout in the multi-repo-components plan); `.ai-dev/tooling/` stays denied unconditionally regardless of any manifest.
